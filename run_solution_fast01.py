    ...
    for _, row in batch_data.iterrows():
        ...
        if len(candidates) < 50 and 'user_activity' in feature_maps:
            ...
            for user_id in feature_maps['voter_activity']:
                if user_id != inviter_id and user_id not in candidates:
                    activity = feature_maps['voter_activity'][user_id]['voter_voted_count'] + \
                               feature_maps['voter_activity'][user_id]['voter_invite_count']
        ...
        if len(candidates) < 50:
            ...
            # 随机选择一些回流者
            random_voters = np.random.choice(remaining_voters, min(50 - len(candidates), len(remaining_voters)),
                                             replace=False)
        ...
        for voter_id in candidates:
            # 构建特征字典
            features = get_features_for_voter(inviter_id, item_id, voter_id, feature_maps, user_user_interactions,
                                              item_cate_map)
...
def generate_predictions_parallel(test_data, model, feature_maps, user_user_interactions, all_voters):
    ...
    try:
        ...
        for i, batch in enumerate(tqdm(batches, desc="生成预测（串行处理）")):
            batch_predictions = process_batch(batch, feature_maps, user_user_interactions, model, all_voters,
                                              item_cate_map)
            predictions.extend(batch_predictions)
