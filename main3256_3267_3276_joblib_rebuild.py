...
def task2(i, testdatas, users, shareitems, itemreceive, responseitems, userSimilarityMatrix):
    submitresAll = []
    pbar = tqdm(testdatas, position=1, desc=f"进程 {i} 正在启动。。。。")
    for testdata in pbar:
        # for testdata in testdatas:
        pbar.desc = "我是进程" + str(i) + ",正在计算的用户是" + testdata['inviter_id']
        # print("我是进程"+str(i)+",正在处理的用户是" + testdata['inviter_id'])
        # 初始化一个名为 res 的 List，用于存储推荐结果
        res = []
        # 提取用户 ID（id1）
        ...
        scor = {}
        # 遍历用户 id1 的相似用户（fid）
        for sid in userSimilarityMatrix[id1]:
            r = userSimilarityMatrix[id1][sid]
            if -r not in scor:
                scor[-r] = []
            scor[-r].append(sid)
            # 二层
            if sid in userSimilarityMatrix:
                for ssid in userSimilarityMatrix[sid]:
                    rr = userSimilarityMatrix[sid][ssid] * r * 0.5
                    if -rr not in scor:
                        scor[-rr] = []
                    scor[-rr].append(ssid)
                # 三层
                if ssid in userSimilarityMatrix:
                    for sssid in userSimilarityMatrix[ssid]:
                        rrr = userSimilarityMatrix[ssid][sssid] * rr * 0.333333333
                        if -rrr not in scor:
                            scor[-rrr] = []
                        scor[-rrr].append(sssid)
        scor = dict(sorted(scor.items()))
        # 对已计算出的评分进行筛选，并将前五名的评分和相关信息记录下来
        kx = -1
        # wrt = f"{id1}\t{itemid}\t{len(shareitems[id1][itemid])}\t{len(users[id1]['Neighbor'].keys())}\t{testdate}\t{len(users[id1]['SimLusers'])}"
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    if verification:
        ...
        print("测试数据加载完成，共有数据：" + str(len(testjsonData)))
    dfData = get_active_data(jsonData)
    # avg
    # 0工作日，1非工作日
    # aKdePdfW = getAKdePdf(df=dfData, isWorkDay=0)
    # aKdePdfH = getAKdePdf(df=dfData, isWorkDay=1)
    # Alist = []
    # Alist.append(aKdePdfW)
    # Alist.append(aKdePdfH)
    # # timeLength = 5
    # # def getPRange(start, end):
    # #     return [
    # #         {
    # #             "i": i,
    # #             "W": getPfromPdf(aKdePdfW, i, timeLength),
    # #             "H": getPfromPdf(aKdePdfH, i, timeLength)
    # #         }
    # #         for i in tqdm(range(start, end),desc=f"进程{start}-{end}",position=1)
    # #     ]
    # #
    # # n_jobs = 4  # 设置你需要的进程数量
    # # total = 60 * 24
    # # chunk_size = total // n_jobs
    # # ranges = [(i, min(total, i + chunk_size)) for i in range(0, total, chunk_size)]
    # # result = Parallel(n_jobs=n_jobs)(delayed(getPRange)(start, end) for start, end in ranges)
    # #
    # # # 将结果展平并排序
    # # PList = [item for sublist in result for item in sublist]
    # # PList.sort(key=lambda x: x['i'])
    # PlistFile = savaPath + "PList.pkl"
    # # diy
    # userHis = []
    # userPList = {}
    ...
    #         userPList = cloudpickle.load(f)
    #         print(userPList)
    # else:
    #     # userHisNum
    #     grouped = dfData.groupby('user_id')
    #     for name, group in tqdm(grouped, desc="用户历史分享时间预处理", position=0):
    #         df_dict = {}
    #         df_dict['user_id'] = name
    #         df_dict['count'] = group.shape[0]
    #         df_dict['timeByMin'] = group['timeByMin'].tolist()
    #         df_dict['workday_count'] = group[group['is_workday_or_holiday'] == 0].shape[0]
    #         df_dict['holiday_count'] = group[group['is_workday_or_holiday'] == 1].shape[0]
    #         userHis.append(df_dict)
    #     userHis = sorted(userHis, key=lambda x: x['count'], reverse=True)
    #
    #     def process_users(i,users, dfData):
    #         userPList = {}
    #         for user in tqdm(users,desc="线程"+str(i)+"用户模型训练",position=1):
    #             userPList[user['user_id']] = {}
    #             if user["workday_count"] <= 3 and user["holiday_count"] <= 3:
    #                 # print("用户"+user['user_id']+"数据不足，跳过")
    #                 continue
    #             userInfo = {}
    #             if user["workday_count"] > 3:
    #                 userInfo['pdfW'] = getDKdePdf(dfData, 0, user['user_id'])
    #             else:
    #                 userInfo['pdfW'] = None
    #             if user["holiday_count"] > 3:
    #                 userInfo['pdfH'] = getDKdePdf(dfData, 1, user['user_id'])
    #             else:
    #                 userInfo['pdfH'] = None
    #             userPList[user['user_id']]=userInfo
    #         return userPList
    #
    #     # 设定多进程的数量
    #     n_jobs = 10
    #
    #     # 拆分 userHis 为 100 份
    #     userHis_splits = np.array_split(userHis, 100)
    #
    #     # 对每份数据进行并行处理
    #     results = Parallel(n_jobs=n_jobs, verbose=50)(
    #         delayed(process_users)(i,userHis_splits[i], dfData) for i in range(len(userHis_splits))
    #     )
    #     userPList = {}
    #     for result in results:
    #         userPList.update(result)
    #     # 保存
    #     with open(PlistFile, 'wb') as f:
    #         cloudpickle.dump(userPList, f)
    ...
    print("变量初始化完成")
    # 遍历用户数据集
    for i in tqdm(range(len(userjsonData)), desc="正在处理用户数据"):
        temp = userjsonData[i]  # 获取用户数据集中的第i个用户数据
    ...
    # 初始化进程所需要的共享数据
    usersMini = {}
    for userId, user in tqdm(users.items(), desc="正在准备多线程数据"):
    ...
    # 开始多进程
    #####################陈老师的原始方案 0.3256####################
    # results = Parallel(n_jobs=threadingNum,verbose=50,backend="loky",prefer="threads")(delayed(task)(i,testjsonDataSplits[i],
    # usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,Alist,userPList) for i in range(len(testjsonDataSplits)))
    results = Parallel(n_jobs=threadingNum, verbose=50, backend="loky", prefer="threads")(
        delayed(task)(i, testjsonDataSplits[i],
                      usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank) for i in
        range(len(testjsonDataSplits)))
    end2 = time.time()
    print("多进程1耗时" + str(end2 - start2) + "秒")
        ...
        for key, sub_dict in d.items():
            ...
            for sub_key, value in sub_dict.items():
                if sub_key in userSimilarityMatrix[key]:
                    ...
                    userSimilarityMatrix[key][sub_key] = value
                userSimilarityMatrix[key][sub_key] = float(userSimilarityMatrix[key][sub_key])
    # 将外层词典的键排序
    ...
    # 使用orjson保存
    with open(savaPath + "userSimilarityMatrix.json", "wb") as f:
        f.write(orjson.dumps(userSimilarityMatrix))
    start3 = time.time()
    results2 = Parallel(n_jobs=threadingNum, verbose=50, backend="loky", prefer="threads")(
        delayed(task2)(i, testjsonDataSplits[i], usersMini, shareitems, itemreceive, responseitems,
                       userSimilarityMatrix) for i in range(len(testjsonDataSplits)))
    end3 = time.time()
    print("多进程2耗时" + str(end3 - start3) + "秒")
    submitresAll = [item for sublist in results2 for item in sublist]
    submitresAll = sorted(submitresAll, key=lambda x: int(x['triple_id']))
    submitres = del_inviter_id(submitresAll)
